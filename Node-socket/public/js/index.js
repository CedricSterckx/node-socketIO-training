...
if (roomId) {
    document.getElementById('start').innerText = `Rejoindre la partie`;
}
...
socket.on('start game', (players) => startGame(players));
...
function startGame(players) {
    ...
    const ennemyPlayer = players.find(p => p.socketId !== player.id);
    ennemyUsername = ennemyPlayer.username;

    if (player.host && player.turn) {
        setTurnMessage('alert-info', 'alert-success', `C'est ton tour de jouer`);
    } else {
        setTurnMessage('alert-success', 'alert-info', `C'est le tour de <b>${ennemyUsername} </b>`);
    }
